...
use crate::AppState;
use crate::utils::{snowflake::DISCORD_SNOWFLAKE, token::verify_token};
...
async fn create_guild(
    ...
    let user_id = verify_token(auth_header, &secret).map_err(|_| StatusCode::UNAUTHORIZED)?;

    // Staff Check
    let user_flags: (i32,) = sqlx::query_as("SELECT public_flags FROM users WHERE id = ?")
        .bind(&user_id)
    ...
    let channels = vec![
        (&text_category_id, ChannelType::GuildCategory as i32, 0, &guild_id, None::<&String>, "Salons textuels"),
        (&voice_category_id, ChannelType::GuildCategory as i32, 1, &guild_id, None, "Salons vocaux"),
        (&system_channel_id, ChannelType::GuildText as i32, 0, &guild_id, Some(&text_category_id), "général"),
        (&afk_channel_id, ChannelType::GuildVoice as i32, 0, &guild_id, Some(&voice_category_id), "Général"),
    ];
    ...
    let overwrite_type = OverwriteType::Role as i32;
    ...
    sqlx::query("INSERT INTO messages (id, timestamp, channel_id, author_id, type) VALUES (?, ?, ?, ?, ?)")
        ...
        .bind(&system_channel_id)
        .bind(&user_id)
        .bind(MessageType::UserJoin as i32)
        .execute(&mut *tx).await.map_err(|e| {
    ...
    let res = serde_json::json!({
        ...
        "channels": [
            { "id": text_category_id, "type": ChannelType::GuildCategory as i32, "name": "Salons textuels", "parent_id": None::<String> },
            { "id": voice_category_id, "type": ChannelType::GuildCategory as i32, "name": "Salons vocaux", "parent_id": None::<String> },
            { "id": system_channel_id, "type": ChannelType::GuildText as i32, "name": "général", "parent_id": text_category_id },
            { "id": afk_channel_id, "type": ChannelType::GuildVoice as i32, "name": "Général", "parent_id": voice_category_id }
        ],
        "roles": [
